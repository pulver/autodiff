...
row (after the header row) of Table~\ref{sqrtnan}, as is mathematically correct. The \nth{2} row in which
...
The resolution in this case is to manually perform the squaring, replacing the \nth{2} row with the \nth{3}:
...
%
%\subsubsection{Avoid Indeterminate Values}
...
\begin{verbatim}
    << "call color  = " <<  call_price.derivative(2,0,1,0) << '\n'
\end{verbatim}
This returns the \nth{2} partial derivative of {\tt call\_price} with respect to variable 1 ({\tt S})
and the \nth{1} partial derivative with respect to variable 3 ({\tt tau}).
