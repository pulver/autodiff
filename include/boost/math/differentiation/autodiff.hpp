...
struct promote_args_n {
  using type = typename tools::promote_args_2<
      RealType, typename promote_args_n<RealTypes...>::type>::type;
};
...
template<typename RealType>
using get_order_sum =
detail::get_order_sum_value<typename std::decay<RealType>::type>;
...
class fvar {
 ...
 public:
  ...
  fvar(const fvar<RealType2, Order2> &);  // NOLINT explicit
  ...
  // static_cast<root_type>(ca) compiles.
  ...
  template<typename RealType2, size_t Order2>
  friend fvar<RealType2, Order2> operator+(const typename fvar<RealType2, Order2>::root_type &, const fvar<RealType2, Order2> &);
  ...
  template<typename RealType2, size_t Order2>
  friend bool operator!=(const typename fvar<RealType2, Order2>::root_type &,
 ...
 private:
  RealType epsilon_inner_product(size_t z0, size_t isum0, size_t m0,
                                 const fvar &cr, size_t z1, size_t isum1,
                                 size_t m1, size_t j) const;
  ...
  template <typename SizeType>
  fvar epsilon_multiply_cpp11(std::false_type, SizeType z0, size_t isum0, const fvar &cr, size_t z1, size_t isum1) const;
...
promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> pow(const fvar<RealType1, Order1> &, const fvar<RealType2, Order2> &);
...
template<typename RealType1, size_t Order1, typename RealType2, size_t Order2>
promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> atan2(const fvar<RealType1, Order1> &, const fvar<RealType2, Order2> &);

// fmod(cr1,cr2) | RealType
template<typename RealType1, size_t Order1, typename RealType2, size_t Order2>
promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> fmod(const fvar<RealType1, Order1> &, const fvar<RealType2, Order2> &);
...
using autodiff_fvar =
typename detail::nest_fvar<RealType, Order, Orders...>::type;
...
fvar<RealType, Order>::fvar(const char *ca_str)
    : v{{static_cast<RealType>(
             boost::lexical_cast<promote<root_type, double>>(ca_str))}} {}
...
fvar<RealType, Order> &fvar<RealType, Order>::operator*=(const fvar<RealType2, Order2> &cr) {
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR(Order <= Order2) {
    for (size_t i = 0, j = Order; i <= Order; ++i, --j) {
      v[j] = std::inner_product(v.cbegin(), v.cend() - ssize_t(i),
                                cr.v.crbegin() + ssize_t(i), zero);
    }
  } else {
    for (size_t i = 0, j = Order; i <= Order - Order2; ++i, --j) {
      v[j] = std::inner_product(cr.v.cbegin(), cr.v.cend(),
                                v.crbegin() + ssize_t(i), zero);
    }
    for (auto i = Order - Order2 + 1, j = Order2 - 1; i <= Order; ++i, --j) {
      v[j] = std::inner_product(cr.v.cbegin(), cr.v.cbegin() + ssize_t(j + 1),
                                v.crbegin() + ssize_t(i), zero);
    }
  }
  return *this;
...
fvar<RealType, Order> &fvar<RealType, Order>::operator/=(const fvar<RealType2, Order2> &cr) {
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR(Order < Order2) {
    for (size_t i = 1, j = Order2 - 1, k = Order; i <= Order; ++i, --j, --k) {
      (v[i] -= std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j),
                                  v.crbegin() + ssize_t(k), zero)) /= cr.v.front();
    }
...
template<typename RealType, size_t Order>
template<typename RealType2, size_t Order2>
promote<fvar<RealType, Order>, fvar<RealType2, Order2>> fvar<RealType, Order>::operator*(const fvar<RealType2, Order2> &cr) const {
  using data_t = typename std::decay<decltype(v)>::type;
  using ssize_t = typename data_t::difference_type;
  const promote<RealType, RealType2> zero(0);
  promote<fvar, fvar<RealType2, Order2>> retval;
...
fvar<RealType, Order>::operator/(const fvar<RealType2, Order2> &cr) const {
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR(Order < Order2) {
    for (size_t i = 1, j = Order2 - 1u; i <= Order; ++i, --j) {
      retval.v[i] = (v[i] - std::inner_product(cr.v.cbegin() + ssize_t(1),
                                               cr.v.cend() - ssize_t(j),
                                               retval.v.crbegin() + ssize_t(j + 1),
                                               zero))/cr.v.front();
    }
    for (size_t i = Order + 1u, j = Order2 - Order - ssize_t(1);
         i <= Order2; ++i, --j) {
      retval.v[i] =
          -std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j),
                              retval.v.crbegin() + ssize_t(j + 1), zero)/cr.v.front();
    }
  } else if BOOST_AUTODIFF_IF_CONSTEXPR(0 < Order2) {
    for (size_t i = 1, j = Order2 - 1, k = Order; i <= Order;
         ++i, j && --j, --k) {
      retval.v[i] = (v[i] - std::inner_product(
          cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j),
...
template<typename RealType, size_t Order>
template<typename RealType2, size_t Order2>
bool fvar<RealType, Order>::operator>=(const fvar<RealType2, Order2> &cr)
const {
  return v.front() >= cr.v.front();
}
...
fvar<RealType, Order> fvar<RealType, Order>::apply(const std::function<root_type(size_t)> &f) const {
  ...
  for (auto i = 1u; i <= order_sum; ++i) {  // accumulator += (epsilon_i *= epsilon) * (f(i) /
    // boost::math::factorial<root_type>(i));
    epsilon_i = epsilon_i.epsilon_multiply(i - 1, 0, epsilon, 1, 0);
    accumulator += epsilon_i.epsilon_multiply(
        i, 0, f(i)/math::factorial<root_type>(i));
  }
  return accumulator;
}

// f : order -> derivative(order)/factorial(order)
// Use this when the computation of the derivatives already includes the
// factorial terms. E.g. See atan().
template<typename RealType, size_t Order>
...
// factorial terms. E.g. See atan().
template<typename RealType, size_t Order>
fvar<RealType, Order> fvar<RealType, Order>::apply_with_horner_factorials(const std::function<root_type(size_t)> &f) const {
  const fvar<RealType, Order> epsilon = fvar<RealType, Order>(*this).set_root(0);
...
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
// Can throw "std::out_of_range: array::at: __n (which is 7) >= _Nm (which is
// 7)"
template<typename RealType, size_t Order>
template<typename... Orders>
...
template<typename RealType, size_t Order>
template<typename... Orders>
get_type_at<fvar<RealType, Order>, sizeof...(Orders)>
fvar<RealType, Order>::derivative(Orders... orders) const {
...
template<typename RealType, size_t Order>
RealType fvar<RealType, Order>::epsilon_inner_product(size_t z0, const size_t isum0, const size_t m0, const fvar<RealType, Order> &cr, size_t z1, const size_t isum1, const size_t m1, const size_t j) const {
  static_assert(is_fvar<RealType>::value,
                "epsilon_inner_product() must have 1 < depth.");
...
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
template<typename RealType, size_t Order>
fvar<RealType, Order> fvar<RealType, Order>::epsilon_multiply(size_t z0, size_t isum0, const fvar<RealType, Order> &cr, size_t z1, std::size_t isum1) const {
  using data_t = typename std::decay<decltype(v)>::type;
  using ssize_t = typename data_t::difference_type;
...
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
// When called from outside this method, z0 should be non-zero. Otherwise if
// z0=0 then it will give an incorrect result of 0 when the root value is 0
// and ca=inf, when instead the correct product is nan. If z0=0 then use the
// regular multiply operator*() instead.
template<typename RealType, size_t Order>
...
fvar<RealType, Order> fvar<RealType, Order>::inverse_apply() const {
  root_type derivatives[order_sum + 1];  // LCOV_EXCL_LINE This causes a false
  // negative on lcov coverage test.
...
fvar<RealType, Order> fabs(const fvar<RealType, Order> &cr) {
  ...
  return cr < zero ? -cr
                   : cr==zero
                     ? fvar<RealType, Order>()  // Canonical fabs'(0) = 0.
                     : cr;                      // Propagate NaN.
}
...
template<typename RealType, size_t Order>
fvar<RealType, Order> pow(const typename fvar<RealType, Order>::root_type &x,
                          const fvar<RealType, Order> &y) {
  using std::log;
  return exp(y*log(x));
}

template<typename RealType1, size_t Order1, typename RealType2,
    size_t Order2>
promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> pow(const fvar<RealType1, Order1> &x, const fvar<RealType2, Order2> &y) {
  return exp(y*log(x));
}
...
fvar<RealType, Order> sqrt(const fvar<RealType, Order> &cr) {
  ...
  constexpr size_t order = fvar<RealType, Order>::order_sum;
  root_type derivatives[order + 1];
  const auto x = static_cast<root_type>(cr);
  *derivatives = sqrt(x);
...
// Natural logarithm. If cr==0 then derivative(i) may have nans due to nans
// from inverse().
template<typename RealType, size_t Order>
...
fvar<RealType, Order> ldexp(const fvar<RealType, Order> &cr, int exp) {
  // argument to std::exp2 must be casted to RealType, otherwise std::exp2
  // returns double (always)
  using std::exp2;
...
fvar<RealType, Order> asin(const fvar<RealType, Order> &cr) {
  ...
  // d1 = sqrt((1-(d1*=d1)).inverse()); // asin(1): d1 =
  // depth(1)(inf,-nan,-nan,-nan,-nan)
  return cr.apply_with_factorials([&d0, &d1](size_t i) { return i ? d1.at(i - 1)/i : d0; });
}
...
template<typename RealType1, size_t Order1, typename RealType2, size_t Order2>
promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> fmod(const fvar<RealType1, Order1> &cr1, const fvar<RealType2, Order2> &cr2) {
  using std::trunc;
  const auto numer =
      static_cast<typename fvar<RealType1, Order1>::root_type>(cr1);
  const auto denom =
      static_cast<typename fvar<RealType2, Order2>::root_type>(cr2);
...
fvar<RealType, Order> lambert_w0(const fvar<RealType, Order> &cr) {
  ...
  *derivatives = lambert_w0(static_cast<root_type>(cr));
  ...
  for (size_t n = 3; n <= order; ++n) {
    ...
    derivatives[n] =
        d1_powers*
            std::accumulate(coef.crend() - ssize_t(n - 1), coef.crend(),
                            coef[n - 1],
                            [&x](const root_type &a, const root_type &b) {
                              return a*x + b;
                            });
  }
  return cr.apply([&derivatives](size_t i) { return derivatives[i]; });
...
fvar<RealType, Order> sinc(const fvar<RealType, Order> &cr) {
  ...
  for (size_t n = 2; n <= order; n += 2) {
    taylor[n] = (1 - (static_cast<int>(n) & 2))/
        math::factorial<root_type>(static_cast<unsigned>(n) + 1);
  }
...
/// boost::math::tools::digits<RealType>() is handled by this
/// std::numeric_limits<> specialization, and similarly for max_value,
/// min_value, log_max_value, log_min_value, and epsilon.
template<typename RealType, size_t Order>
class numeric_limits<
    boost::math::differentiation::detail::fvar<RealType, Order>>
    : public numeric_limits<typename boost::math::differentiation::detail::fvar<
        RealType, Order>::root_type> {
};
...
struct promote_args_2<differentiation::detail::fvar<RealType0, Order0>,
  ...
  using type = differentiation::detail::fvar<
      typename promote_args_2<RealType0, RealType1>::type,
...
struct promote_args_2<differentiation::detail::fvar<RealType0, Order0>,
                      RealType1> {
  using type = differentiation::detail::fvar<
      typename promote_args_2<RealType0, RealType1>::type, Order0>;
};

template<typename RealType0, typename RealType1, size_t Order1>
struct promote_args_2<RealType0,
                      differentiation::detail::fvar<RealType1, Order1>> {
  using type = differentiation::detail::fvar<
      typename promote_args_2<RealType0, RealType1>::type, Order1>;
};
...
DestinationT real_cast(differentiation::detail::fvar<RealType, Order> from_v) {
  using root_type = typename differentiation::detail::get_root_type<
      differentiation::detail::fvar<RealType, Order>>::type;
...
struct evaluation<fvar_t<float, Order>, Policy> {
  using type =
  fvar_t<mp11::mp_if_c<Policy::promote_float_type::value, double, float>,
         Order>;
};
...
struct evaluation<fvar_t<double, Order>, Policy> {
  using type = fvar_t<
      mp11::mp_if_c<Policy::promote_double_type::value, long double, double>,
      Order>;
};
