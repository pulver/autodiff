...
template <typename RealType, typename... RealTypes> struct promote_args_n {
  using type = typename tools::promote_args_2<RealType,
                                              typename promote_args_n<RealTypes...>::type>::type;
};

template <typename RealType> struct promote_args_n<RealType> {
  using type = typename tools::promote_arg<RealType>::type;
};

}  // namespace detail

template <typename RealType, typename... RealTypes> using promote = typename detail::promote_args_n<
  RealType,RealTypes...>::type;

namespace detail {

template <typename RealType, size_t Order> class fvar;

// Compile-time test for fvar<> type.
namespace detail {
template <typename /*RealType*/> struct is_fvar_value : std::false_type {};

template <typename RealType, std::size_t Order> struct is_fvar_value<fvar<RealType,Order>> : std::true_type {};
} // namespace detail

template <typename RealType> using is_fvar = detail::is_fvar_value<typename std::decay<RealType>::type>;

namespace detail {
template <typename... Ts>
using eq_fvars_v = mp11::mp_same<mp11::mp_list<mp11::mp_transform_q<
    mp11::mp_quote<mp11::mp_to_bool>, mp11::mp_transform_q<mp11::mp_quote<is_fvar>, mp11::mp_list<Ts...>>>>>;
...
static_assert(eq_fvars_v<fvar<fvar<double,4>,3>,const fvar<fvar<double,4>,3>>::value, "");
} // namespace detail 

namespace detail {
template <typename /*RealType*/> struct get_depth_value : mp11::mp_size_t<0> {};

template <typename RealType, std::size_t Order> struct get_depth_value<fvar<RealType,Order>>
    : mp11::mp_plus<typename get_depth_value<typename std::decay<RealType>::type>::type,mp11::mp_size_t<1>> {};
} // namespace detail

template <typename RealType> using get_depth = detail::get_depth_value<typename std::decay<RealType>::type>;

namespace detail {
template <typename... Ts> using eq_depth_v = mp11::mp_same<mp11::mp_list<get_depth<Ts>...>>;
...
static_assert(eq_depth_v<fvar<fvar<double,4>,4>,const fvar<const fvar<double,4>,4>>::value, "");
} // namespace detail

namespace detail {
template <typename /*RealType*/> struct get_order_sum_value : mp11::mp_size_t<0> {};

template <typename RealType, std::size_t Order> struct get_order_sum_value<fvar<RealType,Order>>
    : mp11::mp_plus<typename get_order_sum_value<typename std::decay<RealType>::type>::type,mp11::mp_size_t<Order>> {};
} // namespace detail

template <typename RealType> using get_order_sum = detail::get_order_sum_value<typename std::decay<RealType>::type>;
...
static_assert(get_order_sum<fvar<fvar<double, 4>, 4>>::value == get_order_sum<fvar<const fvar<double, 4>, 4>>::value,
              "");
} // namespace detail

// Get non-fvar<> root type T of autodiff_fvar<T,O0,O1,O2,...>.
template <typename RealType> struct get_root_type {
  using type = RealType;
};

template <typename RealType, size_t Order> struct get_root_type<fvar<RealType,Order>> {
...
template <typename RealType, size_t Depth> struct type_at {
  using type = RealType;
};

template <typename RealType, size_t Order, size_t Depth> struct type_at<fvar<RealType,Order>,Depth> {
  using type =
  mp11::mp_if_c<Depth == 0,fvar<RealType,Order>,typename type_at<RealType,Depth - 1>::type>;
};

template <typename RealType, size_t Depth> using get_type_at = typename type_at<RealType,Depth>::type;
...
public:
  ...
  template <typename RealType2, size_t Order2> fvar(const fvar<RealType2,Order2>&); // NOLINT explicit
  ...
  template <typename RealType2, size_t Order2> fvar& operator+=(const fvar<RealType2,Order2>&);
  ...
  template <typename RealType2, size_t Order2> promote<fvar,fvar<RealType2,Order2>> operator+(
    const fvar<RealType2,Order2>&) const;

  // cr + ca | RealType | Binary Addition
  fvar operator+(const root_type&) const;

  // ca + cr | RealType | Binary Addition
  template <typename RealType2, size_t Order2> friend fvar<RealType2,Order2> operator+(
    const typename fvar<RealType2,Order2>::root_type&, const fvar<RealType2,Order2>&);
  ...
  template <typename RealType2, size_t Order2> bool operator!=(const fvar<RealType2,Order2>&) const;

  // cr != ca | bool | Inequality Comparison
  bool operator!=(const root_type&) const;

  // ca != cr | bool | Inequality Comparison
  template <typename RealType2, size_t Order2> friend bool operator!=(const typename fvar<RealType2,Order2>::root_type&,
  ...
  template <typename... Orders> get_type_at<RealType,sizeof...(Orders)> at(size_t order, Orders ... orders) const;

  template <typename... Orders> get_type_at<fvar,sizeof...(Orders)> derivative(Orders ... orders) const;
...
private:
  ...
  template <typename RealType2, size_t Orders2> friend class fvar;

  template <typename RealType2, size_t Order2> friend std::ostream& operator<<(
    std::ostream&, const fvar<RealType2,Order2>&);

// C++11 Compatibility
#ifdef BOOST_NO_CXX17_IF_CONSTEXPR
  template <typename RootType> void fvar_cpp11(std::true_type, const RootType& ca, bool is_variable);

  template <typename RootType> void fvar_cpp11(std::false_type, const RootType& ca, bool is_variable);

  template <typename... Orders> get_type_at<RealType,sizeof...(Orders)> at_cpp11(
    std::true_type, size_t order, Orders ... orders) const;

  template <typename... Orders> get_type_at<RealType,sizeof...(Orders)> at_cpp11(
    std::false_type, size_t order, Orders ... orders) const;

  template <typename SizeType> fvar epsilon_multiply_cpp11(std::true_type,
                                                           SizeType z0, size_t isum0, const fvar& cr, size_t z1,
                                                           size_t isum1) const;

  template <typename SizeType> fvar epsilon_multiply_cpp11(std::false_type,
                                                           SizeType z0, size_t isum0, const fvar& cr, size_t z1,
                                                           size_t isum1) const;

  template <typename SizeType> fvar epsilon_multiply_cpp11(std::true_type, SizeType z0, size_t isum0,
                                                           const root_type& ca) const;

  template <typename SizeType> fvar epsilon_multiply_cpp11(std::false_type, SizeType z0, size_t isum0,
                                                           const root_type& ca) const;

  template <typename RootType> fvar& multiply_assign_by_root_type_cpp11(std::true_type, bool is_root,
                                                                        const RootType& ca);

  template <typename RootType> fvar& multiply_assign_by_root_type_cpp11(std::false_type, bool is_root,
                                                                        const RootType& ca);

  template <typename RootType> fvar& set_root_cpp11(std::true_type, const RootType& root);

  template <typename RootType> fvar& set_root_cpp11(std::false_type, const RootType& root);
#endif
};
...
// fabs(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> fabs(const fvar<RealType,Order>&);

// abs(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> abs(const fvar<RealType,Order>&);

// ceil(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> ceil(const fvar<RealType,Order>&);

// floor(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> floor(const fvar<RealType,Order>&);

// exp(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> exp(const fvar<RealType,Order>&);

// pow(cr, ca) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> pow(const fvar<RealType,Order>&,
                                                                    const typename fvar<RealType,Order>::root_type&);

// pow(ca, cr) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> pow(const typename fvar<RealType,Order>::root_type&,
                                                                    const fvar<RealType,Order>&);

// pow(cr1, cr2) | RealType
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2> promote<
  fvar<RealType1,Order1>,fvar<RealType2,Order2>> pow(const fvar<RealType1,Order1>&, const fvar<RealType2,Order2>&);

// sqrt(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> sqrt(const fvar<RealType,Order>&);

// log(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> log(const fvar<RealType,Order>&);

// frexp(cr1, &i) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> frexp(const fvar<RealType,Order>&, int*);

// ldexp(cr1, i) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> ldexp(const fvar<RealType,Order>&, int);

// cos(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> cos(const fvar<RealType,Order>&);

// sin(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> sin(const fvar<RealType,Order>&);

// asin(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> asin(const fvar<RealType,Order>&);

// tan(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> tan(const fvar<RealType,Order>&);

// atan(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> atan(const fvar<RealType,Order>&);

// atan2(cr1,cr2) | RealType
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2> promote<
  fvar<RealType1,Order1>,fvar<RealType2,Order2>> atan2(const fvar<RealType1,Order1>&, const fvar<RealType2,Order2>&);

// fmod(cr1,cr2) | RealType
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2> promote<
  fvar<RealType1,Order1>,fvar<RealType2,Order2>> fmod(const fvar<RealType1,Order1>&, const fvar<RealType2,Order2>&);

// round(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> round(const fvar<RealType,Order>&);

// iround(cr1) | int
template <typename RealType, size_t Order> int iround(const fvar<RealType,Order>&);

// trunc(cr1) | RealType
template <typename RealType, size_t Order> fvar<RealType,Order> trunc(const fvar<RealType,Order>&);

// itrunc(cr1) | int
template <typename RealType, size_t Order> int itrunc(const fvar<RealType,Order>&);

// Additional functions
template <typename RealType, size_t Order> fvar<RealType,Order> acos(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> acosh(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> asinh(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> atanh(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> cosh(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> erf(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> erfc(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> lambert_w0(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> sinc(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> sinh(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> fvar<RealType,Order> tanh(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> long lround(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> long long llround(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> long long lltrunc(const fvar<RealType,Order>&);

template <typename RealType, size_t Order> long double truncl(const fvar<RealType,Order>&);
...
template <typename RealType, size_t Order> struct nest_fvar<RealType,Order> {
  using type = fvar<RealType,Order>;
};

} // namespace detail

template <typename RealType, size_t Order, size_t... Orders> using autodiff_fvar = typename detail::nest_fvar<
  RealType,Order,Orders...>::type;

template <typename RealType, size_t Order, size_t... Orders> autodiff_fvar<RealType,Order,Orders...> make_fvar(
  const RealType& ca) {
...
template <typename RealType, size_t Order> template <typename RealType2, size_t Order2> fvar<RealType,Order>::fvar(
  const fvar<RealType2,Order2>& cr) {
...
template <typename RealType, size_t Order> fvar<RealType,Order>::fvar(const root_type& ca)
  : v{{static_cast<RealType>(ca)}} {}

template <typename RealType, size_t Order> template <typename RealType2> fvar<RealType,Order>::fvar(const RealType2& ca)
  : v{{static_cast<RealType>(ca)}} // Can cause compiler error if RealType2 cannot be cast to root_type.
{}

template <typename RealType, size_t Order> fvar<RealType,Order>::fvar(const char* ca_str)
...
template <typename RealType, size_t Order> template <typename RealType2, size_t Order2> fvar<RealType,Order>& fvar<
  RealType,Order>::operator+=(const fvar<RealType2,Order2>& cr) {
  for (size_t i = 0 ; i <= (std::min)(Order, Order2) ; ++i)
    v[i] += cr.v[i];
  return *this;
}

template <typename RealType, size_t Order> fvar<RealType,Order>& fvar<RealType,Order>::operator+=(const root_type& ca) {
  v.front() += ca;
  return *this;
}
...
template <typename RealType, size_t Order> template <typename RealType2, size_t Order2> fvar<RealType,Order>& fvar<
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (Order < Order2)
    for (size_t i = 1, j = Order2 - 1, k = Order ; i <= Order ; ++i, --j, --k)
      (v[i] -= std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j), v.crbegin() + ssize_t(k), zero)) /= cr
                                                                                                                   .v.
                                                                                                                   front();
...
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::operator-() const {
  fvar retval;
  for (size_t i = 0 ; i <= Order ; ++i)
    retval.v[i] = -v[i];
  return retval;
...
template <typename RealType, size_t Order> const fvar<RealType,Order>& fvar<RealType,Order>::operator+() const {
  return *this;
}

template <typename RealType, size_t Order> template <typename RealType2, size_t Order2> promote<
  fvar<RealType,Order>,fvar<RealType2,Order2>> fvar<RealType,Order>::operator+(const fvar<RealType2,Order2>& cr) const {
...
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::operator+(
  const root_type& ca) const {
...
template <typename RealType, size_t Order> fvar<RealType,Order> operator+(
  const typename fvar<RealType,Order>::root_type& ca, const fvar<RealType,Order>& cr) {
  return cr + ca;
}

template <typename RealType, size_t Order> template <typename RealType2, size_t Order2> promote<
  fvar<RealType,Order>,fvar<RealType2,Order2>> fvar<RealType,Order>::operator-(
...
template <typename RealType, size_t Order> template <typename RealType2, size_t Order2> promote<
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (Order < Order2) {
    for (size_t i = 1, j = Order2 - 1u ; i <= Order ; ++i, --j)
      retval.v[i] = (v[i] -
                     std::inner_product(cr.v.cbegin() + ssize_t(1), cr.v.cend() - ssize_t(j),
                                        retval.v.crbegin() + ssize_t(j + 1), zero)) / cr.v.front();
    for (size_t i = Order + 1u, j = Order2 - Order - ssize_t(1) ; i <= Order2 ; ++i, --j)
      retval.v[i] =
          -std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j), retval.v.crbegin() + ssize_t(j + 1), zero) /
          cr.v.front();
  } else if BOOST_AUTODIFF_IF_CONSTEXPR (0 < Order2)
    for (size_t i = 1, j = Order2 - 1, k = Order ; i <= Order ; ++i, j && --j, --k)
      retval.v[i] =
      (v[i] - std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j), retval.v.crbegin() + ssize_t(k), zero)
      ) / cr.v.front();
  else
    for (size_t i = 1 ; i <= Order ; ++i)
...
template <typename RealType, size_t Order> fvar<RealType,Order> operator/(
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (0 < Order) {
    ...
    for (size_t i = 1, j = Order - 1 ; i <= Order ; ++i, --j)
      retval.v[i] = -std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j),
                                        retval.v.crbegin() + ssize_t(j + 1), zero)
                    / cr.v.front();
  }
  return retval;
}

template <typename RealType, size_t Order> template <typename RealType2, size_t Order2> bool fvar<RealType,Order>::
operator==(const fvar<RealType2,Order2>& cr) const {
  return fpequal(v.front(), cr.v.front());
}

template <typename RealType, size_t Order> bool fvar<RealType,Order>::operator==(const root_type& ca) const {
  return fpequal(v.front(), ca);
}

template <typename RealType, size_t Order> bool operator==(const typename fvar<RealType,Order>::root_type& ca,
...
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::apply(
  const std::function<root_type(size_t)>& f) const {
  ...
  for (auto i = 1u ; i <= order_sum ; ++i
    ...
    accumulator += epsilon_i.epsilon_multiply(i, 0, f(i) / math::factorial<root_type>(i));
  }
  return accumulator;
}
...
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::apply_with_factorials(
  const std::function<root_type(size_t)>& f) const {
  const fvar epsilon = fvar(*this).set_root(0);
  fvar epsilon_i = fvar(1); // epsilon to the power of i
  fvar accumulator = fvar(f(0));
  for (size_t i = 1 ; i <= order_sum ; ++i) {    // accumulator += (epsilon_i *= epsilon) * f(i);
    epsilon_i = epsilon_i.epsilon_multiply(i - 1, 0, epsilon, 1, 0);
...
// f : order -> derivative(order)
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::apply_with_horner(
  const std::function<root_type(size_t)>& f) const {
  const fvar<RealType,Order> epsilon = fvar<RealType,Order>(*this).set_root(0);
  fvar<RealType,Order> accumulator(static_cast<root_type>(
                                     f(order_sum) / math::factorial<root_type>(static_cast<unsigned>(order_sum))));
  for (auto i = order_sum ; i-- ;) {
    (accumulator *= epsilon) += f(i) / math::factorial<root_type>(static_cast<unsigned>(i));
  }
  return accumulator;
}
...
// f : order -> derivative(order)/factorial(order)
// Use this when the computation of the derivatives already includes the factorial terms. E.g. See atan().
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::apply_with_horner_factorials(
  const std::function<root_type(size_t)>& f) const {
  const fvar<RealType,Order> epsilon = fvar<RealType,Order>(*this).set_root(0);
...
get_type_at<fvar<RealType,Order>,sizeof...(Orders)> fvar<RealType,Order>::derivative(Orders... orders) const
{
    ...
    return at(static_cast<std::size_t>(orders)...) * (... * boost::math::factorial<root_type>(static_cast<unsigned>(orders)));
}
#endif

template <typename RealType, size_t Order> RealType fvar<RealType,Order>::epsilon_inner_product(
  size_t z0, const size_t isum0, const size_t m0,
...
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::inverse() const {
...
template <typename RealType, size_t Order> fvar<RealType,Order> fvar<RealType,Order>::inverse_apply() const {
...
template <typename RealType, size_t Order> fvar<RealType,Order>::operator root_type() const {
  return static_cast<root_type>(v.front());
}

template <typename RealType, size_t Order> fvar<RealType,Order>::operator int() const {
...
template <typename RealType, size_t Order> fvar<RealType,Order> fabs(const fvar<RealType,Order>& cr) {
...
template <typename RealType, size_t Order> fvar<RealType,Order> abs(const fvar<RealType,Order>& cr) {
  return fabs(cr);
}

template <typename RealType, size_t Order> fvar<RealType,Order> ceil(const fvar<RealType,Order>& cr) {
  using std::ceil;
  return fvar<RealType,Order>(ceil(static_cast<typename fvar<RealType,Order>::root_type>(cr)));
}

template <typename RealType, size_t Order> fvar<RealType,Order> floor(const fvar<RealType,Order>& cr) {
  using std::floor;
  return fvar<RealType,Order>(floor(static_cast<typename fvar<RealType,Order>::root_type>(cr)));
}

template <typename RealType, size_t Order> fvar<RealType,Order> exp(const fvar<RealType,Order>& cr) {
  using std::exp;
...
template <typename RealType, size_t Order> fvar<RealType,Order> pow(const fvar<RealType,Order>& x,
                                                                    const typename fvar<RealType,Order>::root_type& y) {
  using std::pow;
  using root_type = typename fvar<RealType,Order>::root_type;
...
template <typename RealType, size_t Order> fvar<RealType,Order> pow(const typename fvar<RealType,Order>::root_type& x,
                                                                    const fvar<RealType,Order>& y) {
  using std::log;
  return exp(y * log(x));
}

template <typename RealType1, size_t Order1, typename RealType2, size_t Order2> promote<
  fvar<RealType1,Order1>,fvar<RealType2,Order2>> pow(const fvar<RealType1,Order1>& x, const fvar<RealType2,Order2>& y) {
  return exp(y * log(x));
}

template <typename RealType, size_t Order> fvar<RealType,Order> sqrt(const fvar<RealType,Order>& cr) {
  using std::sqrt;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
  root_type derivatives[order + 1];
...
template <typename RealType, size_t Order> fvar<RealType,Order> log(const fvar<RealType,Order>& cr) {
  using std::log;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
...
template <typename RealType, size_t Order> fvar<RealType,Order> frexp(const fvar<RealType,Order>& cr, int* exp) {
  using std::exp2;
  using std::frexp;
  using root_type = typename fvar<RealType,Order>::root_type;
...
template <typename RealType, size_t Order> fvar<RealType,Order> ldexp(const fvar<RealType,Order>& cr, int exp) {
...
template <typename RealType, size_t Order> fvar<RealType,Order> cos(const fvar<RealType,Order>& cr) {
  using std::cos;
  using std::sin;
  using root_type = typename fvar<RealType,Order>::root_type;
  ...
  return cr.apply_with_horner([&derivatives](const size_t i) { return derivatives[i % 4]; });
}

template <typename RealType, size_t Order> fvar<RealType,Order> sin(const fvar<RealType,Order>& cr) {
  using std::sin;
  using std::cos;
  using root_type = typename fvar<RealType,Order>::root_type;
...
template <typename RealType, size_t Order> fvar<RealType,Order> asin(const fvar<RealType,Order>& cr) {
  using std::asin;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
  ...
  d1 = sqrt(1 - (d1 *= d1)).
      inverse();                                    // asin(1): d1 = depth(1)(inf,inf,-nan,-nan,-nan)
  //d1 = sqrt((1-(d1*=d1)).inverse()); // asin(1): d1 = depth(1)(inf,-nan,-nan,-nan,-nan)
  return cr.apply_with_factorials([&d0,&d1](size_t i) { return i ? d1.at(i - 1) / i : d0; });
}

template <typename RealType, size_t Order> fvar<RealType,Order> tan(const fvar<RealType,Order>& cr) {
  return sin(cr) / cos(cr);
}

template <typename RealType, size_t Order> fvar<RealType,Order> atan(const fvar<RealType,Order>& cr) {
  using std::atan;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
...
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2> promote<
  fvar<RealType1,Order1>,fvar<RealType2,Order2>> atan2(const fvar<RealType1,Order1>& cr1,
...
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2> promote<
  fvar<RealType1,Order1>,fvar<RealType2,Order2>> fmod(const fvar<RealType1,Order1>& cr1,
                                                      const fvar<RealType2,Order2>& cr2) {
  using std::trunc;
...
template <typename RealType, size_t Order> fvar<RealType,Order> round(const fvar<RealType,Order>& cr) {
  using std::round;
  return fvar<RealType,Order>(round(static_cast<typename fvar<RealType,Order>::root_type>(cr)));
}

template <typename RealType, size_t Order> int iround(const fvar<RealType,Order>& cr) {
  using math::iround;
  return iround(static_cast<typename fvar<RealType,Order>::root_type>(cr));
}

template <typename RealType, size_t Order> fvar<RealType,Order> trunc(const fvar<RealType,Order>& cr) {
  using std::trunc;
  return fvar<RealType,Order>(trunc(static_cast<typename fvar<RealType,Order>::root_type>(cr)));
}

template <typename RealType, size_t Order> int itrunc(const fvar<RealType,Order>& cr) {
  using math::itrunc;
  return itrunc(static_cast<typename fvar<RealType,Order>::root_type>(cr));
}

template <typename RealType, size_t Order> std::ostream& operator<<(std::ostream& out, const fvar<RealType,Order>& cr) {
  out << "depth(" << cr.depth << ')';
...
template <typename RealType, size_t Order> fvar<RealType,Order> acos(const fvar<RealType,Order>& cr) {
  using std::acos;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
...
template <typename RealType, size_t Order> fvar<RealType,Order> acosh(const fvar<RealType,Order>& cr) {
  using std::acosh;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
...
template <typename RealType, size_t Order> fvar<RealType,Order> asinh(const fvar<RealType,Order>& cr) {
  using std::asinh;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
...
template <typename RealType, size_t Order> fvar<RealType,Order> atanh(const fvar<RealType,Order>& cr) {
  using std::atanh;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
...
template <typename RealType, size_t Order> fvar<RealType,Order> cosh(const fvar<RealType,Order>& cr) {
  using std::cosh;
  using std::sinh;
  using root_type = typename fvar<RealType,Order>::root_type;
...
template <typename RealType, size_t Order> fvar<RealType,Order> erf(const fvar<RealType,Order>& cr) {
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (order == 0)
    return fvar<RealType,Order>(d0);
  auto x = make_fvar<root_type,order - 1>(static_cast<root_type>(cr));
  const auto d1 = 2 * constants::one_div_root_pi<root_type>() * exp(-(x *= x)); // 2/sqrt(pi)*exp(-x*x)
  return cr.apply_with_horner_factorials([&d0,&d1](size_t i) { return i ? d1.at(i - 1) / i : d0; });
}

template <typename RealType, size_t Order> fvar<RealType,Order> erfc(const fvar<RealType,Order>& cr) {
  BOOST_MATH_STD_USING
  using std::erfc;
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (order == 0)
    return fvar<RealType,Order>(d0);
  auto x = make_fvar<root_type,order - 1>(static_cast<root_type>(cr));
  const auto d1 = -2 * constants::one_div_root_pi<root_type>() * exp(-(x *= x)); // erfc'(x)=-erf'(x)
  return cr.apply_with_horner_factorials([&d0,&d1](size_t i) { return i ? d1.at(i - 1) / i : d0; });
}

template <typename RealType, size_t Order> fvar<RealType,Order> lambert_w0(const fvar<RealType,Order>& cr) {
  ...
  *derivatives = lambert_w0(static_cast<root_type>(cr));
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (order == 1)
    return cr.apply([&derivatives](size_t i) { return derivatives[i]; });
  root_type d1_powers = derivatives[1] * derivatives[1];
  const root_type x = derivatives[1] * expw;
  derivatives[2] = d1_powers * (-1 - x);
...
template <typename RealType, size_t Order> fvar<RealType,Order> sinc(const fvar<RealType,Order>& cr) {
  if (cr != 0)
    return sin(cr) / cr;
  using root_type = typename fvar<RealType,Order>::root_type;
  constexpr size_t order = fvar<RealType,Order>::order_sum;
  ...
  for (size_t n = 2 ; n <= order ; n += 2)
    taylor[n] = (1 - (static_cast<int>(n) & 2)) / math::factorial<root_type>(static_cast<unsigned>(n) + 1);
  return cr.apply_with_factorials([&taylor](size_t i) { return taylor[i]; });
}

template <typename RealType, size_t Order> fvar<RealType,Order> sinh(const fvar<RealType,Order>& cr) {
  using std::sinh;
  using std::cosh;
  using root_type = typename fvar<RealType,Order>::root_type;
  ...
  return cr.apply_with_horner([&derivatives](const size_t i) { return derivatives[i % 2]; });
}

template <typename RealType, size_t Order> fvar<RealType,Order> tanh(const fvar<RealType,Order>& cr) {
...
template <typename RealType, size_t Order> long lround(const fvar<RealType,Order>& cr) {
  using std::lround;
  return lround(static_cast<typename fvar<RealType,Order>::root_type>(cr));
}

template <typename RealType, size_t Order> long long llround(const fvar<RealType,Order>& cr) {
  using std::llround;
  return llround(static_cast<typename fvar<RealType,Order>::root_type>(cr));
}

template <typename RealType, size_t Order> long long lltrunc(const fvar<RealType,Order>& cr) {
  using math::lltrunc;
  return lltrunc(static_cast<typename fvar<RealType,Order>::root_type>(cr));
}

template <typename RealType, size_t Order> long double truncl(const fvar<RealType,Order>& cr) {
  using std::truncl;
...
template <typename RealType, size_t Order> class numeric_limits<boost::math::differentiation::detail::fvar<
      RealType,Order>>
...
template <typename RealType0, size_t Order0, typename RealType1, size_t Order1> struct promote_args_2<
      differentiation::detail::fvar<RealType0,Order0>,
...
template <typename RealType0, size_t Order0, typename RealType1> struct promote_args_2<
      differentiation::detail::fvar<RealType0,Order0>,RealType1> {
  using type = differentiation::detail::fvar<typename promote_args_2<RealType0,RealType1>::type,Order0>;
};

template <typename RealType0, typename RealType1, size_t Order1> struct promote_args_2<
      RealType0,differentiation::detail::fvar<RealType1,Order1>> {
  using type = differentiation::detail::fvar<typename promote_args_2<RealType0,RealType1>::type,Order1>;
};

template <typename DestinationT, typename RealType, std::size_t Order> DestinationT real_cast(
  differentiation::detail::fvar<RealType,Order> from_v) {
...
template <class Policy, std::size_t Order> struct evaluation<fvar_t<float,Order>,Policy> {
  using type = fvar_t<mp11::mp_if_c<Policy::promote_float_type::value,double,float>,Order>;
};

template <class Policy, std::size_t Order> struct evaluation<fvar_t<double,Order>,Policy> {
  using type = fvar_t<mp11::mp_if_c<Policy::promote_double_type::value,long double,double>,Order>;
};
