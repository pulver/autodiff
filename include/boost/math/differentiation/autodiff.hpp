...
template <typename RealType> struct get_root_type { using type = RealType; };
...
template <typename RealType, size_t Depth> struct type_at { using type = RealType; };

template <typename RealType, size_t Order, size_t Depth> struct type_at<fvar<RealType, Order>, Depth> {
  using type = mp11::mp_if_c<Depth == 0, fvar<RealType, Order>, typename type_at<RealType, Depth - 1>::type>;
};
...
template <typename RealType, size_t Order> class fvar {
 ...
 public:
  ...
  template <typename RealType2, size_t Order2>
  friend fvar<RealType2, Order2> operator+(const typename fvar<RealType2, Order2>::root_type&,
 ...
 private:
  RealType epsilon_inner_product(size_t z0, size_t isum0, size_t m0, const fvar& cr, size_t z1, size_t isum1, size_t m1,
                                 size_t j) const;
...
// C++11 Compatibility
  ...
  fvar epsilon_multiply_cpp11(std::true_type, SizeType z0, size_t isum0, const fvar& cr, size_t z1, size_t isum1) const;

  template <typename SizeType>
  fvar epsilon_multiply_cpp11(std::false_type, SizeType z0, size_t isum0, const fvar& cr, size_t z1,
                              size_t isum1) const;
...
template <typename RealType, size_t Order> struct nest_fvar<RealType, Order> { using type = fvar<RealType, Order>; };
...
template <typename RealType, size_t Order> fvar<RealType, Order>::fvar(const root_type& ca, const bool is_variable) {
  if constexpr (is_fvar<RealType>::value) {
    v.front() = RealType(ca, is_variable);
    if constexpr (0 < Order) std::fill(v.begin() + 1, v.end(), static_cast<RealType>(0));
  } else {
    v.front() = ca;
    if constexpr (0 < Order) v[1] = static_cast<root_type>(static_cast<int>(is_variable));
    if constexpr (1 < Order) std::fill(v.begin() + 2, v.end(), static_cast<RealType>(0));
  }
}
#endif
...
fvar<RealType, Order>::fvar(const fvar<RealType2, Order2>& cr) {
  for (size_t i = 0; i <= (std::min)(Order, Order2); ++i) v[i] = static_cast<RealType>(cr.v[i]);
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(Order2 < Order)
  std::fill(v.begin() + (Order2 + 1), v.end(), static_cast<RealType>(0));
}

template <typename RealType, size_t Order>
fvar<RealType, Order>::fvar(const root_type& ca) : v{{static_cast<RealType>(ca)}} {}
...
fvar<RealType, Order>& fvar<RealType, Order>::operator+=(const fvar<RealType2, Order2>& cr) {
  for (size_t i = 0; i <= (std::min)(Order, Order2); ++i) v[i] += cr.v[i];
  return *this;
}
...
template <typename RealType2, size_t Order2>
fvar<RealType, Order>& fvar<RealType, Order>::operator-=(const fvar<RealType2, Order2>& cr) {
  for (size_t i = 0; i <= Order; ++i) v[i] -= cr.v[i];
  return *this;
}
...
fvar<RealType, Order>& fvar<RealType, Order>::operator*=(const fvar<RealType2, Order2>& cr) {
  ...
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(Order <= Order2)
...
fvar<RealType, Order>& fvar<RealType, Order>::operator/=(const fvar<RealType2, Order2>& cr) {
  ...
  for (size_t i = 1, j = Order2 - 1, k = Order; i <= Order; ++i, --j, --k)
    (v[i] -= std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j), v.crbegin() + ssize_t(k), zero)) /=
        cr.v.front();
  ...
  else for (size_t i = 1; i <= Order; ++i) v[i] /= cr.v.front();
  return *this;
}
...
template <typename RealType, size_t Order> fvar<RealType, Order> fvar<RealType, Order>::operator-() const {
  fvar retval;
  for (size_t i = 0; i <= Order; ++i) retval.v[i] = -v[i];
  return retval;
}
...
promote<fvar<RealType, Order>, fvar<RealType2, Order2>> fvar<RealType, Order>::operator+(
  ...
  for (size_t i = 0; i <= (std::min)(Order, Order2); ++i) retval.v[i] = v[i] + cr.v[i];
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(Order < Order2)
  for (auto i = Order + 1; i <= Order2; ++i) retval.v[i] = cr.v[i];
  else if BOOST_AUTODIFF_IF_CONSTEXPR(Order2 < Order) for (auto i = Order2 + 1; i <= Order; ++i) retval.v[i] = v[i];
  return retval;
}
  ...
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(Order < Order2) {
      for (size_t i = 1, j = Order2 - 1u; i <= Order; ++i, --j)
        retval.v[i] = (v[i] - std::inner_product(cr.v.cbegin() + ssize_t(1), cr.v.cend() - ssize_t(j),
    ...
    BOOST_AUTODIFF_IF_CONSTEXPR(0 < Order2)
  for (size_t i = 1, j = Order2 - 1, k = Order; i <= Order; ++i, j && --j, --k)
    retval.v[i] = (v[i] - std::inner_product(cr.v.cbegin() + 1, cr.v.cend() - ssize_t(j),
                                             retval.v.crbegin() + ssize_t(k), zero)) /
                  cr.v.front();
  else for (size_t i = 1; i <= Order; ++i) retval.v[i] = v[i] / cr.v.front();
  return retval;
}
...
fvar<RealType, Order> operator/(const typename fvar<RealType, Order>::root_type& ca, const fvar<RealType, Order>& cr) {
  ...
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(0 < Order) {
      const RealType zero(0);
...
fvar<RealType, Order> fvar<RealType, Order>::apply(const std::function<root_type(size_t)>& f) const {
  ...
  for (auto i = 1u; i <= order_sum;
       ++i) {  // accumulator += (epsilon_i *= epsilon) * (f(i) / boost::math::factorial<root_type>(i));
...
template <typename RealType, size_t Order>
fvar<RealType, Order> fvar<RealType, Order>::apply_with_factorials(const std::function<root_type(size_t)>& f) const {
  const fvar epsilon = fvar(*this).set_root(0);
  fvar epsilon_i = fvar(1);  // epsilon to the power of i
  fvar accumulator = fvar(f(0));
...
fvar<RealType, Order> fvar<RealType, Order>::apply_with_horner(const std::function<root_type(size_t)>& f) const {
  ...
  fvar<RealType, Order> accumulator(
      static_cast<root_type>(f(order_sum) / math::factorial<root_type>(static_cast<unsigned>(order_sum))));
...
// Use this when the computation of the derivatives already includes the factorial terms. E.g. See atan().
template <typename RealType, size_t Order>
fvar<RealType, Order> fvar<RealType, Order>::apply_with_horner_factorials(
...
get_type_at<fvar<RealType, Order>, sizeof...(Orders)> fvar<RealType, Order>::derivative(Orders... orders) const {
  ...
  return at(static_cast<std::size_t>(orders)...) *
         (... * boost::math::factorial<root_type>(static_cast<unsigned>(orders)));
}
#endif

template <typename RealType, size_t Order>
RealType fvar<RealType, Order>::epsilon_inner_product(size_t z0, const size_t isum0, const size_t m0,
                                                      const fvar<RealType, Order>& cr, size_t z1, const size_t isum1,
                                                      const size_t m1, const size_t j) const {
...
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
template <typename RealType, size_t Order>
fvar<RealType, Order> fvar<RealType, Order>::epsilon_multiply(size_t z0, size_t isum0, const fvar<RealType, Order>& cr,
                                                              size_t z1, size_t isum1) const {
  using data_t = typename std::decay<decltype(v)>::type;
  using ssize_t = typename data_t::difference_type;
...
template <typename RealType, size_t Order>
fvar<RealType, Order> fvar<RealType, Order>::epsilon_multiply(size_t z0, size_t isum0, const root_type& ca) const {
  fvar<RealType, Order> retval(*this);
  const size_t m0 = order_sum + isum0 < Order + z0 ? Order + z0 - (order_sum + isum0) : 0;
  if constexpr (is_fvar<RealType>::value) {
...
template <typename RealType, size_t Order> fvar<RealType, Order> sqrt(const fvar<RealType, Order>& cr) {
  ...
  *derivatives = sqrt(x);
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(order == 0)
...
template <typename RealType, size_t Order> fvar<RealType, Order> cos(const fvar<RealType, Order>& cr) {
  ...
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(fvar<RealType, Order>::order_sum == 0) { return fvar<RealType, Order>(d0); }
...
std::ostream& operator<<(std::ostream& out, const fvar<RealType, Order>& cr) {
  out << "depth(" << cr.depth << ')';
  for (size_t i = 0; i < cr.v.size(); ++i) out << (i ? ',' : '(') << cr.v[i];
  return out << ')';
}
...
template <typename RealType, size_t Order> fvar<RealType, Order> cosh(const fvar<RealType, Order>& cr) {
  ...
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(fvar<RealType, Order>::order_sum == 0)
  return fvar<RealType, Order>(d0);
...
template <typename RealType, size_t Order> fvar<RealType, Order> lambert_w0(const fvar<RealType, Order>& cr) {
  ...
  *derivatives = lambert_w0(static_cast<root_type>(cr));
  ...
  if
    BOOST_AUTODIFF_IF_CONSTEXPR(order == 1)
  return cr.apply([&derivatives](size_t i) { return derivatives[i]; });
