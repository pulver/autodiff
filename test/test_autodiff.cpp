...
BOOST_AUTO_TEST_CASE_TEMPLATE(mixed_partials, T, all_float_types) {
  ...
  // Derivatives calculated from symbolic differentiation by Mathematica for
  // comparison.
...
BOOST_AUTO_TEST_CASE_TEMPLATE(multiprecision, T, bin_float_types)
{
  ...
  const auto y = make_fvar<T,0,0,Ny>(13);
  const auto z = make_fvar<T,0,0,0,Nz>(14);
  const auto v = mixed_partials_f(w,x,y,z); // auto =
autodiff_fvar<T,Nw,Nx,Ny,Nz>
  // Calculated from Mathematica symbolic differentiation.
  const T answer =
boost::lexical_cast<T>("1976.31960074779771777988187529041872090812118921875499076582535951111845769110560421820940516423255314");
  // BOOST_REQUIRE_CLOSE(v.derivative(Nw,Nx,Ny,Nz), answer, eps); // Doesn't
work for cpp_dec_float using std::fabs; const double relative_error =
static_cast<double>(fabs(v.derivative(Nw,Nx,Ny,Nz)/answer-1));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(black_scholes, T, bin_float_types) {
  ...
  // Compare automatically calculated greeks by autodiff with formulas for
  // greeks.
  // https://en.wikipedia.org/wiki/Greeks_(finance)#Formulas_for_European_option_Greeks
  ...
  // intermediate cpp_dec_float calculation can't go to template function as it
  // can't be implicitly cast back to T.
  ...
  // const T formula_call_theta =
  // static_cast<T>(-S*phi(d1)*sigma/(2*sqrt(tau))-r*K*exp(-r*tau)*Phi(+d2));
  const T formula_call_theta = static_cast<T>(-S * phi(d1) * sigma / (2 * sqrt(tau)) - r * K * exp(-r * tau) * Phi_pd2);
  // const T formula_put_theta  =
  // static_cast<T>(-S*phi(d1)*sigma/(2*sqrt(tau))+r*K*exp(-r*tau)*Phi(-d2));
  ...
  const T formula_charm =
      static_cast<T>(phi(d1) * (d2 * sigma * sqrt(tau) - 2 * r * tau) / (2 * tau * sigma * sqrt(tau)));
  const T formula_vomma = static_cast<T>(S * phi(d1) * sqrt(tau) * d1 * d2 / sigma);
  const T formula_veta =
      static_cast<T>(-S * phi(d1) * sqrt(tau) * (r * d1 / (sigma * sqrt(tau)) - (1 + d1 * d2) / (2 * tau)));
  ...
  BOOST_REQUIRE_CLOSE(-call_price.derivative(0, 0, 1, 0), formula_call_theta,
                      eps);  // minus sign from tau = T-time
