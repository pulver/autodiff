...
struct test_constants_t<T, std::integral_constant<Order, Val>> {
  static constexpr int n_samples =
      mp11::mp_if<is_multiprecision_t<T>,
                  mp11::mp_int<10>, mp11::mp_int<25>>::value;
  static constexpr Order order = Val;
  static constexpr int mp_epsilon_multiplier = mp11::mp_if<is_multiprecision_t<T>, mp11::mp_int<2>, mp11::mp_int<1>>::value;
...
constexpr bool check_if_small(const LHS& lhs, const RHS& rhs) noexcept {
  using boost::math::differentiation::detail::get_root_type;
  using boost::math::differentiation::detail::is_fvar;
  using promoted_real_type_t = promote<LHS, RHS>;

  return std::numeric_limits<promoted_real_type_t>::epsilon() >
         fabs((std::max)(static_cast<promoted_real_type_t>(lhs),
                         static_cast<promoted_real_type_t>(rhs)) -
              (std::min)(static_cast<promoted_real_type_t>(lhs),
                         static_cast<promoted_real_type_t>(rhs)));
}
