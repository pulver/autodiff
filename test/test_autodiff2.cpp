...
BOOST_AUTO_TEST_CASE_TEMPLATE(exp_test, T, all_float_types) {
  ...
  for (auto i : boost::irange(m + 1)) {
    ...
    BOOST_CHECK_CLOSE_FRACTION(y.derivative(i), exp(cx), std::numeric_limits<T>::epsilon());
  }
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(pow, T, bin_float_types) {
  ...
  BOOST_CHECK_EQUAL(z0.derivative(3u), cy * (cy - 1) * (cy - 2) * pow(cx, cy - 3));
  ...
  BOOST_CHECK_CLOSE(z2.derivative(2u, 0u), pow(cx, cy - 2) * cy * (cy - 1), eps);
  BOOST_CHECK_CLOSE(z2.derivative(2u, 1u), pow(cx, cy - 2) * (cy * (cy - 1) * log(cx) + 2 * cy - 1), eps);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(pow2, T, bin_float_types) {
  ...
  // Mathematica: Export["pow.csv", Flatten@Table[ Simplify@D[x^y,{x,i},{y,j}] /. {x->2, y->5/2},
  //                    { i, 0, 5 }, { j, 0, 5 } ] ]
  ...
  const T mathematica[] {
    ...
    static_cast<T>(2*root_two<T>()*ln_two<T>()*(2 + (5*ln_two<T>())/2)),
    static_cast<T>(2*root_two<T>()*pow(ln_two<T>(),2)*(3 + (5*ln_two<T>())/2)),
    static_cast<T>(2*root_two<T>()*pow(ln_two<T>(),3)*(4 + (5*ln_two<T>())/2)),
    static_cast<T>(2*root_two<T>()*pow(ln_two<T>(),4)*(5 + (5*ln_two<T>())/2)),
    static_cast<T>(15/(2*root_two<T>())),
    static_cast<T>(root_two<T>()*(4 + (15*ln_two<T>())/4)),
    static_cast<T>(root_two<T>()*(2 + 8*ln_two<T>() + (15*pow(ln_two<T>(),2))/4)),
    static_cast<T>(root_two<T>()*ln_two<T>()*(6 + 12*ln_two<T>() + (15*pow(ln_two<T>(),2))/4)),
    static_cast<T>(root_two<T>()*pow(ln_two<T>(),2)*(12 + 16*ln_two<T>() + (15*pow(ln_two<T>(),2))/4)),
    static_cast<T>(root_two<T>()*pow(ln_two<T>(),3)*(20 + 20*ln_two<T>() + (15*pow(ln_two<T>(),2))/4)),
    static_cast<T>(15/(8*root_two<T>())),
    static_cast<T>((23/4.0 + (15*ln_two<T>())/8)/root_two<T>()),
    static_cast<T>((9 + (23*ln_two<T>())/2 + (15*pow(ln_two<T>(),2))/8)/root_two<T>()),
    static_cast<T>((6 + 27*ln_two<T>() + (69*pow(ln_two<T>(),2))/4 + (15*pow(ln_two<T>(),3))/8)/root_two<T>()),
    static_cast<T>((ln_two<T>()*(24 + 54*ln_two<T>() + 23*pow(ln_two<T>(),2) + (15*pow(ln_two<T>(),3))/8))/root_two<T>()),
    static_cast<T>((pow(ln_two<T>(),2)*(60 + 90*ln_two<T>() + (115*pow(ln_two<T>(),2))/4 + (15*pow(ln_two<T>(),3))/8))/root_two<T>()),
    static_cast<T>(-15/(32*root_two<T>())),
    static_cast<T>((-1 - (15*ln_two<T>())/16)/(2*root_two<T>())),
    static_cast<T>((7 - 2*ln_two<T>() - (15*pow(ln_two<T>(),2))/16)/(2*root_two<T>())),
    static_cast<T>((24 + 21*ln_two<T>() - 3*pow(ln_two<T>(),2) - (15*pow(ln_two<T>(),3))/16)/(2*root_two<T>())),
    static_cast<T>((24 + 96*ln_two<T>() + 42*pow(ln_two<T>(),2) - 4*pow(ln_two<T>(),3) - (15*pow(ln_two<T>(),4))/16)/(2*root_two<T>())),
    static_cast<T>((ln_two<T>()*(120 + 240*ln_two<T>() + 70*pow(ln_two<T>(),2) - 5*pow(ln_two<T>(),3) - (15*pow(ln_two<T>(),4))/16))/(2*root_two<T>())),
    static_cast<T>(45/(128*root_two<T>())),
    static_cast<T>((9/16.0 + (45*ln_two<T>())/32)/(4*root_two<T>())),
    static_cast<T>((-25/2.0 + (9*ln_two<T>())/8 + (45*pow(ln_two<T>(),2))/32)/(4*root_two<T>())),
    static_cast<T>((-15 - (75*ln_two<T>())/2 + (27*pow(ln_two<T>(),2))/16 + (45*pow(ln_two<T>(),3))/32)/(4*root_two<T>())),
    static_cast<T>((60 - 60*ln_two<T>() - 75*pow(ln_two<T>(),2) + (9*pow(ln_two<T>(),3))/4 + (45*pow(ln_two<T>(),4))/32)/(4*root_two<T>())),
    static_cast<T>((120 + 300*ln_two<T>() - 150*pow(ln_two<T>(),2) - 125*pow(ln_two<T>(),3) + (45*pow(ln_two<T>(),4))/16 + (45*pow(ln_two<T>(),5))/32)/(4*root_two<T>())) };
  std::size_t k=0;
  for (auto i : boost::irange(m + 1)) {
    for (auto j : boost::irange(n + 1)) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(sqrt_test, T, all_float_types) {
  ...
  auto y = sqrt(x);
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(0u), sqrt(cx), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(1u), 0.5 * pow(cx, -0.5), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(2u), -0.5 * 0.5 * pow(cx, -1.5), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(3u), 0.5 * 0.5 * 1.5 * pow(cx, -2.5), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(4u), -0.5 * 0.5 * 1.5 * 2.5 * pow(cx, -3.5),
                               std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(5u), 0.5 * 0.5 * 1.5 * 2.5 * 3.5 * pow(cx, -4.5),
                               std::numeric_limits<T>::epsilon());
  x = make_fvar<T, m>(0);
  y = sqrt(x);
  ...
  for (auto i : boost::irange(std::size_t(1), m + 1)) {
    BOOST_CHECK_EQUAL(y.derivative(i), (i % 2 == 1 ? 1 : -1) * std::numeric_limits<T>::infinity());
  }
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(log_test, T, all_float_types) {
  ...
  auto x = make_fvar<T, m>(cx);
  auto y = log(x);
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(0u), log(cx), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(1u), 1 / cx, std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(2u), -1 / pow(cx, 2), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(3u), 2 / pow(cx, 3), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(4u), -6 / pow(cx, 4), std::numeric_limits<T>::epsilon());
  BOOST_CHECK_CLOSE_FRACTION(y.derivative(5u), 24 / pow(cx, 5), std::numeric_limits<T>::epsilon());
  x = make_fvar<T, m>(0);
  y = log(x);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(ylogx, T, all_float_types) {
  ...
  for (auto i : boost::irange(1u, static_cast<unsigned>(m + 1))) {
    BOOST_CHECK_CLOSE(z.derivative(i, 0u), pow(-1, i - 1) * boost::math::factorial<T>(i - 1) * cy / pow(cx, i), eps);
    BOOST_CHECK_CLOSE(z.derivative(i, 1u), pow(-1, i - 1) * boost::math::factorial<T>(i - 1) / pow(cx, i), eps);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(acos_test, T, bin_float_types) {
  ...
  BOOST_CHECK_CLOSE(y.derivative(3u), -(2 * cx * cx + 1) / pow(1 - cx * cx, 2.5), eps);
  BOOST_CHECK_CLOSE(y.derivative(4u), -3 * cx * (2 * cx * cx + 3) / pow(1 - cx * cx, 3.5), eps);
  BOOST_CHECK_CLOSE(y.derivative(5u), -(24 * (cx * cx + 3) * cx * cx + 9) / pow(1 - cx * cx, 4.5), eps);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(acosh_test, T, bin_float_types) {
  ...
  BOOST_CHECK_CLOSE(y.derivative(1u), 1 / boost::math::constants::root_three<T>(), eps);
  BOOST_CHECK_CLOSE(y.derivative(2u), -2 / (3 * boost::math::constants::root_three<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(3u), 1 / boost::math::constants::root_three<T>(), eps);
  BOOST_CHECK_CLOSE(y.derivative(4u), -22 / (9 * boost::math::constants::root_three<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(5u), 227 / (27 * boost::math::constants::root_three<T>()), 2*eps);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(asin_derivative, T, bin_float_types) {
  ...
  BOOST_CHECK_CLOSE(y.derivative(4u), -(12 * cx * cx + 3) / pow(1 - cx * cx, 3.5), eps);
  ...
  BOOST_CHECK_CLOSE(y.derivative(2u), (2 * cx * cx + 1) / pow(1 - cx * cx, 2.5), eps);
  BOOST_CHECK_CLOSE(y.derivative(3u), 3 * cx * (2 * cx * cx + 3) / pow(1 - cx * cx, 3.5), eps);
  BOOST_CHECK_CLOSE(y.derivative(4u), (24 * (cx * cx + 3) * cx * cx + 9) / pow(1 - cx * cx, 4.5), eps);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(asinh_test, T, bin_float_types) {
  ...
  BOOST_CHECK_CLOSE(y.derivative(1u), 1 / boost::math::constants::root_two<T>(), eps);
  BOOST_CHECK_CLOSE(y.derivative(2u), -1 / (2 * boost::math::constants::root_two<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(3u), 1 / (4 * boost::math::constants::root_two<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(4u), 3 / (8 * boost::math::constants::root_two<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(5u), -39 / (16 * boost::math::constants::root_two<T>()), eps);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(atan2_function, T, all_float_types) {
  ...
  for (auto i : boost::irange(test_constants::n_samples)) {
    ...
    BOOST_CHECK_CLOSE(autodiff_v, anchor_v, 5000 * test_constants::pct_epsilon());
  }
}
