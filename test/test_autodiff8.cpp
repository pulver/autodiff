...
BOOST_AUTO_TEST_CASE_TEMPLATE(heuman_lambda_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		...
		BOOST_CHECK_CLOSE(
			...
			.derivative(0u),
			boost::math::heuman_lambda(x, phi),
			250 * test_constants::pct_epsilon());
	}
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(hypot_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		...
		BOOST_CHECK_CLOSE(boost::math::hypot(make_fvar<T, m>(x), make_fvar<T, m>(y))
			.derivative(0u),
			boost::math::hypot(x, y),
			50 * test_constants::pct_epsilon());
	}
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(jacobi_zeta_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		...
		BOOST_CHECK_CLOSE(
			...
			1.25e4 * test_constants::pct_epsilon());
	}
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(laguerre_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
...

		{
			...
			BOOST_TEST_WARN(isNearZero(autodiff_v.derivative(0) - anchor_v));
		}
		{
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
...

		{
			...
			BOOST_CHECK_CLOSE(
				...
				boost::math::lambert_wm1(x_), 50 * test_constants::pct_epsilon());
			BOOST_CHECK_CLOSE(
...
BOOST_AUTO_TEST_CASE_TEMPLATE(next_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		BOOST_CHECK_CLOSE(
			float_next(make_fvar<T, m>(static_cast<T>(i))).derivative(0u),
			float_next(static_cast<T>(i)), test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(
			float_prior(make_fvar<T, m>(static_cast<T>(i))).derivative(0u),
			float_prior(static_cast<T>(i)), test_constants::pct_epsilon());

		BOOST_CHECK_CLOSE(nextafter(make_fvar<T, m>(static_cast<T>(i)),
			make_fvar<T, m>(static_cast<T>(1))),
			nextafter(static_cast<T>(i), static_cast<T>(1)),
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(nextafter(make_fvar<T, m>(static_cast<T>(i)),
			make_fvar<T, m>(static_cast<T>(i + 2))),
			nextafter(static_cast<T>(i), static_cast<T>(i + 2)),
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(nextafter(make_fvar<T, m>(static_cast<T>(i)),
			make_fvar<T, m>(static_cast<T>(i + 1)))
			.derivative(0u),
			nextafter(static_cast<T>(i), static_cast<T>(i + 2)),
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(nextafter(make_fvar<T, m>(static_cast<T>(i)),
			...
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(nextafter(make_fvar<T, m>(static_cast<T>(i)),
			make_fvar<T, m>(static_cast<T>(-1 * (i + 2))))
			.derivative(0u),
			nextafter(static_cast<T>(i), -1 * static_cast<T>(i + 2)),
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(nextafter(make_fvar<T, m>(static_cast<T>(i)),
			make_fvar<T, m>(static_cast<T>(-1 * (i + 1))))
			.derivative(0u),
			nextafter(static_cast<T>(i), -1 * static_cast<T>(i + 2)),
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(nextafter(make_fvar<T, m>(static_cast<T>(i)),
		...
		BOOST_CHECK_CLOSE(
			float_advance(make_fvar<T, m>(static_cast<T>(i)), 1).derivative(0u),
			float_advance(static_cast<T>(i), 1), test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(
			float_advance(make_fvar<T, m>(static_cast<T>(i)), i + 2).derivative(0u),
			float_advance(static_cast<T>(i), i + 2), test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(
			float_advance(make_fvar<T, m>(static_cast<T>(i)), i + 1).derivative(0u),
			float_advance(float_advance(make_fvar<T, m>(static_cast<T>(i)), i + 2),
				-1)
			.derivative(0u),
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(
			...
			float_advance(static_cast<T>(i), -1), test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(
			float_advance(make_fvar<T, m>(static_cast<T>(i)), -i - 1)
			.derivative(0u),
			float_advance(float_advance(make_fvar<T, m>(static_cast<T>(i)), -i - 2),
				1)
			.derivative(0u),
			test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(float_advance(make_fvar<T, m>(static_cast<T>(i)), 0),
			((make_fvar<T, m>(static_cast<T>(i)))),
			test_constants::pct_epsilon());

		BOOST_CHECK_CLOSE(
			float_distance(make_fvar<T, m>(static_cast<T>(i)).derivative(0u),
				static_cast<T>(i)),
			static_cast<T>(0), test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(
			float_distance(float_next(make_fvar<T, m>(static_cast<T>(i))),
				make_fvar<T, m>(static_cast<T>(i)))
			.derivative(0u),
			((make_fvar<T, m>(-1))).derivative(0u), test_constants::pct_epsilon());
		BOOST_CHECK_CLOSE(
			float_distance(float_prior(make_fvar<T, m>(static_cast<T>(i))),
				make_fvar<T, m>(static_cast<T>(i)))
			.derivative(0u),
			((make_fvar<T, m>(1))).derivative(0u), test_constants::pct_epsilon());
	}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(polygamma_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		...
		try {
			...
			std::cout << "Overflow Error thrown with inputs i: " << i << " x: " << x << std::endl;
			BOOST_CHECK_THROW(boost::math::polygamma(i, make_fvar<T, m>(x)), boost::wrapexcept<std::overflow_error>);
			BOOST_CHECK_THROW(boost::math::polygamma(i, x), boost::wrapexcept<std::overflow_error>);
		}
	}
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(powm1_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		...
		BOOST_CHECK_CLOSE(autodiff_v.derivative(0u), anchor_v,
			2e6 * test_constants::pct_epsilon());
	}
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(sin_pi_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		std::ignore = i;
		auto x = x_sampler.next();
		BOOST_CHECK_CLOSE(boost::math::sin_pi(make_fvar<T, m>(x)).derivative(0u),
			boost::math::sin_pi(x), test_constants::pct_epsilon());
	}
}
	...
	for (auto n : boost::irange(
		...
		auto r = (std::min)(static_cast<int>(n) - 1, r_sampler.next());
		{
			...
			BOOST_CHECK_CLOSE(autodiff_v.real(), anchor_v.real(),
				2.5e4 * test_constants::pct_epsilon());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(sqrt1pm1_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		std::ignore = i;
		auto x = x_sampler.next();
		BOOST_CHECK_CLOSE(boost::math::sqrt1pm1(make_fvar<T, m>(x)),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(trigamma_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		std::ignore = i;
		auto x = x_sampler.next();
		BOOST_CHECK_CLOSE(boost::math::trigamma(make_fvar<T, m>(x)),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(zeta_hpp, T, all_float_types) {
	...
	for (auto i : boost::irange(test_constants::n_samples)) {
		std::ignore = i;
		auto x = x_sampler.next();
		BOOST_CHECK_CLOSE(boost::math::zeta(make_fvar<T, m>(x)),
			boost::math::zeta(x),
