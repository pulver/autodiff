...
lcov --gcov-tool=gcov-7 --base-directory "$BOOST_ROOT/libs/$SELF" --directory "$BOOST_ROOT" --capture --output-file all.info
...
cat /tmp/interesting
xargs -L 999999 -a /tmp/interesting lcov --gcov-tool=gcov-7 --extract all.info {} "*/libs/$SELF/src/*" --output-file coverage.info
