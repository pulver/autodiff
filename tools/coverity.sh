...
if [[ "$1" != "--skipdownload" ]]; then
  ...
  curl --cacert /tmp/scanca.cer -L -d "token=$COVERITY_SCAN_TOKEN&project=$COVERITY_PROJECT_NAME" -X POST https://scan.coverity.com/download/linux64 -o coverity_tool.tgz
